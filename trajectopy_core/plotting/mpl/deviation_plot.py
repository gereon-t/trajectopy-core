...
from trajectopy_core.plotting.mpl.heatmap import annotate_heatmap, heatmap
from trajectopy_core.plotting.mpl.utils import (
    UNIT_DICT,
    XLABEL_DICT,
...
from trajectopy_core.settings.mpl_plotting import MPLPlotSettings

logger = logging.getLogger("root")


def plot_combined_devs(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()) -> Figure:
...
def plot_raw_rotation_devs(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()) -> Figure:
    vert_sp_data_rot = [
        [
...
def plot_raw_position_devs(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()) -> Figure:
    deviations_xa = (
...
def plot_dof_dev(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()) -> Figure:
...
def plot_position_dof(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()):
    deviations_xa = (
        devs.abs_dev.directed_pos_dev[:, 0] if plot_settings.show_directed_devs else devs.abs_dev.pos_dev[:, 0]
    )
    deviations_yh = (
...
def plot_rot_dof(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()) -> None:
    xyz_dev = [devs.trajectory.pos.xyz]
    plt.subplot(2, 3, 4)
    scatter_plotter(
        xyz=xyz_dev,
...
def plot_compact_hist(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()) -> Figure:
    """
...
def plot_rotation_hist(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()) -> None:
...
def plot_position_hist(devs: ATEResult, plot_settings: MPLPlotSettings = MPLPlotSettings()):
    deviations_xa = (
        devs.abs_dev.directed_pos_dev[:, 0] if plot_settings.show_directed_devs else devs.abs_dev.pos_dev[:, 0]
    )
    deviations_yh = (
...
def plot_rotation_rms_heatmap(
    deviation_collection: DeviationCollection, plot_settings: MPLPlotSettings = MPLPlotSettings()
) -> Union[Figure, None]:
