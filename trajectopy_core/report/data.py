from dataclasses import field, dataclass
from functools import cached_property
from typing import Optional

import numpy as np

from trajectopy_core.evaluation.ate_result import ATEResult
from trajectopy_core.evaluation.rpe_result import RPEResult
from trajectopy_core.settings.report import ReportSettings
from trajectopy_core.utils.datahandling import shrink_data


@dataclass
class ReportData:
    """
    Class to store all data needed to render the report.

    Args:
        ate_result: The ATE result to be rendered.
        rpe_result: The RPE result to be rendered.
        settings: The report settings.

    """

    ate_result: ATEResult
    rpe_result: Optional[RPEResult] = None
    settings: ReportSettings = field(default_factory=ReportSettings)

    def __post_init__(self) -> None:
        if self.settings.ate_unit_is_mm:
            self.ate_result.abs_dev.pos_dev *= 1000.0
            self.ate_result.abs_dev.directed_pos_dev *= 1000.0

    @property
    def ate_unit(self) -> str:
        return "mm" if self.settings.ate_unit_is_mm else "m"

    @property
    def has_ate_orientation(self) -> bool:
        return self.ate_result.has_orientation

    @property
    def has_rpe(self) -> bool:
        return self.rpe_result is not None

    @property
    def function_of_label(self) -> str:
        return self.ate_result.trajectory.function_of_label

    @cached_property
    def pos_x(self) -> np.ndarray:
        return shrink_data(self.ate_result.trajectory.pos.x, self.settings.downsample_size)

    @cached_property
    def pos_y(self) -> np.ndarray:
        return shrink_data(self.ate_result.trajectory.pos.y, self.settings.downsample_size)

    @cached_property
    def pos_z(self) -> np.ndarray:
        return shrink_data(self.ate_result.trajectory.pos.z, self.settings.downsample_size)

    @cached_property
    def function_of(self) -> np.ndarray:
        return shrink_data(self.ate_result.trajectory.function_of, self.settings.downsample_size)

    @cached_property
    def comb_dev_pos(self) -> np.ndarray:
        return shrink_data(self.ate_result.comb_pos_devs, self.settings.downsample_size)

    @cached_property
    def pos_dev_x(self) -> np.ndarray:
        return (
            shrink_data(self.ate_result.along, self.settings.downsample_size)
            if self.settings.directed_ate
            else shrink_data(self.ate_result.abs_dev.pos_dev[:, 0], self.settings.downsample_size)
        )

    @cached_property
    def pos_dev_y(self) -> np.ndarray:
        return (
            shrink_data(self.ate_result.cross_h, self.settings.downsample_size)
            if self.settings.directed_ate
            else shrink_data(self.ate_result.abs_dev.pos_dev[:, 1], self.settings.downsample_size)
        )

    @cached_property
    def pos_dev_z(self) -> np.ndarray:
        return (
            shrink_data(self.ate_result.cross_v, self.settings.downsample_size)
            if self.settings.directed_ate
            else shrink_data(self.ate_result.abs_dev.pos_dev[:, 2], self.settings.downsample_size)
        )

    @property
    def pos_dev_x_name(self) -> str:
        return "along" if self.settings.directed_ate else "x"

    @property
    def pos_dev_y_name(self) -> str:
        return "cross-h" if self.settings.directed_ate else "y"

    @property
    def pos_dev_z_name(self) -> str:
        return "cross-v" if self.settings.directed_ate else "z"

    @cached_property
    def roll(self) -> np.ndarray:
        if not self.ate_result.has_orientation:
            raise ValueError("ATE result has no orientation.")

        return np.rad2deg(shrink_data(self.ate_result.trajectory.rpy[:, 0], self.settings.downsample_size))

    @cached_property
    def pitch(self) -> np.ndarray:
        if not self.ate_result.has_orientation:
            raise ValueError("ATE result has no orientation.")

        return np.rad2deg(shrink_data(self.ate_result.trajectory.rpy[:, 1], self.settings.downsample_size))

    @cached_property
    def yaw(self) -> np.ndarray:
        if not self.ate_result.has_orientation:
            raise ValueError("ATE result has no orientation.")

        return np.rad2deg(shrink_data(self.ate_result.trajectory.rpy[:, 2], self.settings.downsample_size))

    @cached_property
    def comb_dev_rot(self) -> np.ndarray:
        if not self.ate_result.has_orientation:
            raise ValueError("ATE result has no orientation.")

        return np.rad2deg(shrink_data(self.ate_result.comb_rot_devs, self.settings.downsample_size))

    @cached_property
    def rot_dev_x(self) -> np.ndarray:
        if not self.ate_result.has_orientation:
            raise ValueError("ATE result has no orientation.")

        return np.rad2deg(shrink_data(self.ate_result.roll, self.settings.downsample_size))

    @cached_property
    def rot_dev_y(self) -> np.ndarray:
        if not self.ate_result.has_orientation:
            raise ValueError("ATE result has no orientation.")

        return np.rad2deg(shrink_data(self.ate_result.pitch, self.settings.downsample_size))

    @cached_property
    def rot_dev_z(self) -> np.ndarray:
        if not self.ate_result.has_orientation:
            raise ValueError("ATE result has no orientation.")

        return np.rad2deg(shrink_data(self.ate_result.yaw, self.settings.downsample_size))
